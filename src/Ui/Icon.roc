module [view, home, userCircle, doorOpen, checkmark, arrowLeft, spinner, adjustmentsHorizontal, xMark]

import Html

view : Str -> Html.Node
view = Html.dangerouslyIncludeUnescapedHtml

Icon : { class ? Str } -> Html.Node

icon : Str -> Icon
icon = \rawHtml -> \{ class ? "size-6" } ->
        rawHtml
        |> Str.replaceFirst "size-6" class
        |> view
...
doorOpen = icon
    """
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" fill="currentColor" class="size-6">
...
spinner = icon
    """
    <svg xmlns='http://www.w3.org/2000/svg' fill='currentColor' stroke='currentColor' viewBox='0 0 24 24' class='size-6'>
        <path
            fill-rule='evenodd'
